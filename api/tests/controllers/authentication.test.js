...
describe("/tokens", () => {
  ...
  test("returns a token when credentials are valid", async () => {
    ...
    const response = await testApp
      .post("/tokens")
      .send({ name: "someone", email: "auth-test@test.com", password: "12345678" });
  ...
  test("doesn't return a token when the user doesn't exist", async () => {
    ...
    const response = await testApp
      .post("/tokens")
      .send({ name: "someone", email: "non-existent@test.com", password: "1234" });
  ...
  test("doesn't return a token when the wrong password is given", async () => {
    ...
    const response = await testApp
      .post("/tokens")
      .send({ name: "someone", email: "auth-test@test.com", password: "1234" });
