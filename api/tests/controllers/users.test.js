...
describe("/users", () => {
  ...
  describe("POST, when username, email and password are provided", () => {
    test("the response code is 201", async () => {
      const response = await request(app)
        .post("/users")
        .send({ username: "name123", email: "poppy@email.com", password: "Password1!" });
    ...
    test("a user is created", async () => {
      await request(app)
        .post("/users")
        .send({ username: "name123", email: "scarconstt@email.com", password: "Password1!" });
  ...
  describe("POST, when username is missing", () => {
    ...
    test("does not create a user", async () => {
      await request(app).post("/users").send({ email: "skye@email.com", password: "Password1!" });
  ...
  describe("POST, when email is missing", () => {
    ...
    test("does not create a user", async () => {
      await request(app).post("/users").send({ username: "user123", password: "Password1!" });
  ...
  describe("POST, when email already exists", () => {
    test("response code is 409", async () => {
      await request(app)
        .post("/users")
        .send({ username: "123", email: "123@email.com", password: "Password1!" });

      const response = await request(app)
        .post("/users")
        .send({ username: "user123", email: "123@email.com", password: "Password1!" });
...
describe("POST, when username already exists", () => {
  test("response code is 409", async () => {
    await request(app)
      .post("/users")
      .send({ username: "user123", email: "user@email.com", password: "Password1!" });
  ...
  test("does not create a user", async () => {
    ...
    await request(app).post("/users").send({ username: "user123", email: "123@email.com", password: "Password1!" });
  ...
  describe("POST, when email is not valid", () => {
    test("response code is 400", async () => {
      response = await request(app)
        .post("/users")
        .send({ username: "user123", email: "123", password: "Password1!" });
